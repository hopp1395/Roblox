...
- **`src/ReplicatedStorage/Shared/GameConfig.lua`** — a single table of all tunable constants (road/obstacle/bonus geometry, speed/accel/steer physics, sound asset ids). Required by both server and client. When balancing gameplay, change values here rather than hardcoding numbers elsewhere.
...
- **Per-player lanes**: each player gets their own isolated parallel track ("lane"), offset along X by `GameConfig.LANE_SPACING`. Players don't share a road or interact with each other — this is really N single-player tracks running side by side. Lanes, obstacles, and bonuses are created lazily and cached in `lanes[laneIndex]`.
- **Streaming world generation**: road segments, obstacles, and bonus items are generated ahead of the car (`ensureRoadAhead`, `ensureObstaclesAhead`, `ensureBonusAhead`, all keyed off `ROAD_LOOKAHEAD`/`OBSTACLE_LOOKAHEAD`) and culled behind it (`OBSTACLE_CULL_DISTANCE`) inside `updatePlayerState`, which runs once per player per Heartbeat tick.
...
- **Collision/scoring** happens in `updatePlayerState`: obstacles behind the car award a point and get destroyed; obstacles within `CAR_HALF_WIDTH`/`CAR_HALF_LENGTH` (scaled) of the car trigger `crashPlayer` and halt that player's state permanently. Bonus items work the same way but add variable points instead of crashing.
